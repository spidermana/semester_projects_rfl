...
Especially, in Rust for Linux, \texttt{unsafe} blocks are an unavoidable necessity: the fundamental mismatch between Rust's strict ownership model and C's type system---in particular around pointer aliasing, validity, and lifetime management---means that no safe abstraction can be built without an
